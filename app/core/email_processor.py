...
class EmailProcessor:
    ...
    def connect(self) -> None:
        ...
        try:
            self.client = IMAPClient(
                self.settings.ZOHO_IMAP_SERVER,
                port=self.settings.ZOHO_IMAP_PORT,
                use_uid=True,
            )
        ...
        try:
            ...
            if invoice_info:
                ...
                if "emisor" in invoice_info:
                    ...
                    invoice_info["emisor_nombre"] = (
                        emisor_parts[1].strip() if len(emisor_parts) > 1 else ""
                    )
    ...
    def _process_email(self, email_message: email.message.Message) -> Optional[dict]:
        ...
        try:
            ...
            print(
                "Procesando correo con asunto:",
                email_message.get("subject", "Sin asunto"),
            )
            ...
            if client_info and invoice_info and xml_url:
                return {
                    "client": {"nit": client_info[0], "nombre": client_info[1]},
