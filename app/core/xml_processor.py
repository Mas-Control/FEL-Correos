...
class XMLProcessor:
    ...
    def _create_factura(
        self, datos_emision: Dict, certificacion: Dict, xml_path: str
    ) -> Factura:
        ...
        receptor = Receptor(
            ...
            direccion=self._get_direccion(
                receptor_data.get("dte:DireccionReceptor", {})
            ),
        )

        # Procesar items
        items = self._process_items(
            datos_emision.get("dte:Items", {}).get("dte:Item", [])
        )

        # Obtener totales
        totales = datos_emision.get("dte:Totales", {})
        total_impuestos = totales.get("dte:TotalImpuestos", {})
        iva = total_impuestos.get("dte:TotalImpuesto", {}).get(
        ...
        return Factura(
            ...
            fecha_emision=datetime.fromisoformat(
                datos_generales.get("@FechaHoraEmision", "")
                .replace("T", " ")
                .split(".")[0]
            ),
                ...
                Item(
                    ...
                    impuestos={
                        ...
                        "monto_gravable": Decimal(
                            impuestos.get("dte:MontoGravable", "0")
                        ),
                        "monto_impuesto": Decimal(
                            impuestos.get("dte:MontoImpuesto", "0")
                        ),
                    },
                )
            )
    ...
    def _save_processed_data(self, factura: Factura) -> None:
        ...
        try:
            ...
            client_dir = os.path.join(
                self.settings.PROCESSED_DATA_PATH, factura.receptor.nit
            )
            ...
            if isinstance(factura_dict.get("fecha_emision"), datetime):
                factura_dict["fecha_emision"] = factura_dict[
                    "fecha_emision"
                ].isoformat()
